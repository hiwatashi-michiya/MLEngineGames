...
void GameManager::Update(bool isJustTurned, bool isJustMoved, bool isReceive, bool isReflect) {
	...
	case GameManager::GameState::Tutorial:
		...
		case GameManager::TutorialState::FlontBack:
			...
			if (turnCount_ >= turnCountMax_) {
				tutorialClearSE_.Play(Audio::SEVolume);
				tuState_ = TutorialState::Receive;
				//isTutorialClear_ = true;
			}
			break;
		case GameManager::TutorialState::Receive:

 			if (isReceive) {
				receiveCount_++;
			}

			if (receiveCount_ >= receiveCountMax_) {
				tutorialClearSE_.Play(Audio::SEVolume);
				tuState_ = TutorialState::Reflect;
			}


			break;
		case GameManager::TutorialState::Reflect:
			if (isReflect) {
				reflectCount_++;
			}
